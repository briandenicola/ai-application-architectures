...
resource "azapi_resource" "nemotron_managed_compute" {
  type                      = "Microsoft.CognitiveServices/accounts/managedComputeDeployments@2026-05-15-preview"
  ...
  body = {
    sku = {
      name     = "GlobalManagedCompute"
      capacity = var.foundry_project.models[1].capacity
    }
    properties = {
      acceleratorType    = var.foundry_project.models[1].sku
      deploymentTemplate = var.foundry_project.models[1].template
      model              = var.foundry_project.models[1].model
    }
  }
  ...
  depends_on = [
    azapi_resource.ai_foundry_project,
    azapi_resource.arctic_embed_managed_compute,
  ]
}
